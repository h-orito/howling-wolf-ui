...
export default class MessageCard extends Vue {
  ...
  private handleCopyAnchorString(): void {
    ...
    const isPaste: boolean =
      villageUserSettings.getOperation(this).is_paste_anchor
    if (isPaste) {
  ...
  private async clickAnchorMessage(anchorString: string): Promise<void> {
    ...
    if (
      this.anchorMessages.some((mes) => isSameMessage(mes, typeCode, number))
    ) {
    ...
    const anchorMessage: VillageAnchorMessage | null =
      await this.loadAnchorMessage(typeCode, number)
